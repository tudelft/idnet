...
def load_flow(flowfile: Path, valid_in_3rd_channel: bool, write_format=WriteFormat):
    ...
    assert channel3.max(
    ) <= 1, f'Maximum value in last channel should be 1: {flowfile}'
...
def check_submission(submission_dir: Path, flow_timestamps_dir: Path):
    ...
    for seq in submission_dir.iterdir():
        ...
        for prediction in seq.iterdir():
            flow, valid_map = load_flow(
                prediction, valid_in_3rd_channel=False, write_format=WriteFormat.IMAGEIO)
        ...
        assert num_files == name2num[
            seq.name], f'expected {name2num[seq.name]} files in {str(seq)} but only found {num_files} files'
...
if __name__ == '__main__':
    ...
    parser.add_argument('flow_timestamps_dir',
                        help='Path to directory containing the flow timestamps for evaluation.')
